...
func main() {
	...
	if _, err := os.Stat(modelPath); os.IsNotExist(err) {
		...
		// 尝试查找其他可用的模型文件
		files, _ := filepath.Glob(filepath.Join(currentDir, "models", "go_params", "*.json"))
		if len(files) > 0 {
			modelPath = files[0]
			fmt.Printf("使用替代模型文件: %s\n", modelPath)
		} else {
			fmt.Println("找不到任何模型文件")
			return
		}
	}

	// 创建OTP检测器
	fmt.Printf("正在加载OTP检测模型...\n")
	otpDetector, err := detector.NewBalancedEnhancedOTPDetector(modelPath)
	if err != nil {
		fmt.Printf("创建OTP检测器失败: %v\n", err)
		return
	}
...
func testExamples(detector *detector.BalancedEnhancedOTPDetector) {
	...
	for i, example := range examples {
		...
		fmt.Printf("消息: %s\n", example.message)
		fmt.Printf("OTP得分: %.4f\n", score)
		if threshold, ok := debugInfo["decision_threshold"].(float64); ok {
			fmt.Printf("阈值: %.4f\n", threshold)
		}
		fmt.Printf("检测结果: %v\n", isOTP)
