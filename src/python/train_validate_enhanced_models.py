...
def evaluate_model(pipeline, val_messages, val_labels, output_dir, model_type):
    ...
    if val_scores is not None:
        ...
        plt.savefig(os.path.join(output_dir, "validation", f"{model_type}_enhanced_roc_curve.png"))
        ...
        plt.savefig(os.path.join(output_dir, "validation", f"{model_type}_enhanced_pr_curve.png"))
        ...
        # 阈值-F1分数曲线
        plt.figure(figsize=(10, 8))
        plt.plot(thresholds, f1_scores[:-1], color='green', lw=2)  # 去掉最后一个点，因为precision_recall_curve返回的thresholds比precision和recall少一个
        plt.axvline(x=best_threshold, color='red', linestyle='--')
        plt.xlabel('Threshold')
        plt.ylabel('F1 Score')
        plt.title(f'Threshold vs F1 Score - {model_type.upper()} Model')
        plt.savefig(os.path.join(output_dir, "validation", f"{model_type}_enhanced_threshold_f1.png"))
...
def main():
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='训练和验证增强特征工程OTP检测模型')
    ...
    for model_type in model_types:
        ...
        # 评估模型
        best_threshold, best_f1 = evaluate_model(pipeline, val_messages, val_labels, output_dir, model_type)
    ...
    for result in results:
        print(f"模型: {result['model_type'].upper()}, 特征工程: {result['balance_method']}, "
