...
def create_model(model_type):
    ...
    if model_type == 'svm':
        # 创建线性SVM分类器，调整OTP类别的权重
        classifier = LinearSVC(
            C=10.0,  # 正则化参数
            class_weight={0: 1, 1: 5},  # 由于数据集已平衡，降低OTP类别的权重
            dual=False,
            max_iter=10000
        )
        print("使用SVM模型")
...
def train_model(train_messages, train_labels, val_messages, val_labels, model_type):
    """训练OTP检测模型"""
    # 预处理文本
    print("预处理文本...")
...
def export_model_params(pipeline, output_dir, processed_messages, model_type, balance_method):
    """导出模型参数为JSON格式"""
    # 获取向量化器
    ...
    tfidf_path = os.path.join(output_dir, f"otp_tfidf_{model_type}_{balance_method}_params.json")
...
def parse_arguments():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description='使用平衡后的数据集训练OTP检测模型')
    parser.add_argument('--model', type=str, default='svm', choices=['svm', 'nb', 'rf'],
                        help='指定模型类型: svm (支持向量机), nb (朴素贝叶斯), rf (随机森林)')
    parser.add_argument('--balance', type=str, default='smote', choices=['smote', 'undersample'],
                        help='使用的数据平衡方法: smote (过采样), undersample (欠采样)')
...
def main():
    ...
    if args.data is None:
        if balance_method == 'smote':
            input_path = os.path.join(base_dir, "data", "processed", "balanced_smote_dataset.csv")
        else:
            input_path = os.path.join(base_dir, "data", "processed", "balanced_undersampled_dataset.csv")
    else:
    ...
    # 加载数据集
    train_messages, train_labels, val_messages, val_labels = load_data_from_csv(input_path)
    
    # 训练模型
    pipeline, processed_train_messages = train_model(train_messages, train_labels, val_messages, val_labels, model_type)
    
    # 导出模型参数
    export_model_params(pipeline, output_dir, processed_train_messages, model_type, balance_method)
