...
func main() {
	...
	// 创建改进的OTP检测器
	fmt.Println("正在加载改进的SVM模型...")
	improvedDetector, err := detector.NewImprovedOTPDetector(modelPath)
	if err != nil {
		log.Fatalf("创建改进的OTP检测器失败: %v", err)
	}

	// 设置调试模式
	improvedDetector.EnableDebug(debug)

	// 根据模式执行不同的操作
	if benchmark {
		runImprovedBenchmark(improvedDetector)
	} else if messageFile != "" {
		processImprovedMessageFile(improvedDetector, messageFile)
	} else if interactive {
		runImprovedInteractive(improvedDetector)
	} else {
		// 默认运行示例
		runImprovedExamples(improvedDetector)
	}
}

// 运行示例
func runImprovedExamples(detector *detector.ImprovedOTPDetector) {
	fmt.Println("===== 示例消息测试 (改进SVM模型) =====")
...
// 交互模式
func runImprovedInteractive(detector *detector.ImprovedOTPDetector) {
	fmt.Println("===== 交互模式 (改进SVM模型) =====")
...
// 处理消息文件
func processImprovedMessageFile(detector *detector.ImprovedOTPDetector, filePath string) {
	fmt.Printf("===== 处理文件 (改进SVM模型): %s =====\n", filePath)
...
// 运行基准测试
func runImprovedBenchmark(detector *detector.ImprovedOTPDetector) {
	fmt.Println("===== 基准测试 (改进SVM模型) =====")
