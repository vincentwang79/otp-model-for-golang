...
func main() {
	...
	svmDetector, err := detector.NewSVMDetector(modelPath)
	if err != nil {
		log.Fatalf("创建SVM检测器失败: %v", err)
	}

	// 设置调试模式
	svmDetector.EnableDebug(debug)

	// 根据模式执行不同的操作
	if benchmark {
		runSVMBenchmark(svmDetector)
	} else if messageFile != "" {
		processSVMMessageFile(svmDetector, messageFile)
	} else if interactive {
		runSVMInteractive(svmDetector)
	} else {
		// 默认运行示例
		runSVMExamples(svmDetector)
	}
}

// 运行示例
func runSVMExamples(detector *detector.SVMDetector) {
...
// 交互模式
func runSVMInteractive(detector *detector.SVMDetector) {
...
// 处理消息文件
func processSVMMessageFile(detector *detector.SVMDetector, filePath string) {
...
// 运行基准测试
func runSVMBenchmark(detector *detector.SVMDetector) {
